...
class FriendsScreen extends BaseScreen {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_.isEqualWith(nextProps?.friends, friends)) {
      console.log(
        TAG,
        ' componentWillReceiveProps01 = length ',
        listFriends.length
      );
  ...
  onPressNextGo = this.onClickView(() => {
    ...
    this.props.navigation.navigate(CHALLENGENAME, {
      invitemode: true,
      mapId: mapId,
      loop: loop,
      miles: sumMiles
  ...
  renderItem = ({ item, index }) => {
    return (
      <ItemFriend
        key={String(index)}
        dataItem={item}
        inviteMode={this.state.inviteMode}
        selectIdfn={this.getIdInviteFriend}
      />
    );
  };
      ...
      >
        ...
        {listFriends.length == 0 && selectedIndex === 0 ? (
          <View
            style={[
              styles.containerImg,
              { marginLeft: verticalScale(40), marginRight: verticalScale(10) }
            ]}
          >
            <Image
              ...
              style={[
                styles.image,
                { resizeMode: 'cover', marginTop: -40, height: 101, width: 150 }
              ]}
            />
            <Text
              style={[
